...
export default function(pool) {
  ...
  router.post('/generate', async (req, res) => {
    ...
    try {
      let questions;
      try {
        const { generateQuiz } = await import('../services/geminiService.js');
        questions = await generateQuiz({ topic, difficulty, count: safeCount });
      } catch (e) {
        // Fallback mock questions if Gemini not configured
        questions = Array.from({ length: safeCount }, (_, i) => ({
          id: i + 1,
          question: `Sample question ${i + 1} about ${topic}?`,
          options: ['Option A', 'Option B', 'Option C', 'Option D'],
          correct: 0,
          explanation: 'Configure GEMINI_API_KEY in .env for real AI-generated questions.'
        }));
      }
